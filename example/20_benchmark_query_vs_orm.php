 ...
 */
function benchmark(string $label, callable $callback, int $iterations = 100): array
{
    $start = hrtime(true);
...
$results = [
    benchmark('Raw query builder', function (): void {
    ...
    benchmark('ORM query builder', function (): void {
        User::query()
            ->where('active', '=', 1)
            ->orderBy('id', 'desc')
            ->limit(25)
...
<head>
    <meta charset="utf-8">
    <title>Query vs ORM Benchmark</title>
    <style>
...
<body>
    <h1>Query vs ORM Benchmark</h1>
    <p>Run this script a few times to smooth out warm-up noise.</p>
